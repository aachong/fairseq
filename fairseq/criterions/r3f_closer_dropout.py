...
@register_criterion('r3f_closer_dropout')
class R3fCloserDropout(FairseqCriterion):
    def __init__(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if model.training:
            noised_logits, _ ,noised_feature= model(
                **sample["net_input"]
            )
            # noised_logits, _ = model(
            #     **sample["net_input"], token_embeddings=noised_embeddings
            # )
            # symm_mse = F.mse_loss(input_feature, noised_feature)
            symm_mse = self._get_symm_kl(input_feature, noised_feature)
