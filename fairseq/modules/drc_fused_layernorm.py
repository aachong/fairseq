...
class HeadFusedLayerNorm(torch.nn.Module):
    ...
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True):
        super(HeadFusedLayerNorm, self).__init__()
    ...
    def forward(self, input):
        self.weight = torch.cat(tuple(self._weight for i in range(64)),dim=-1)
        self.bias = torch.cat(tuple(self._bias for i in range(64)),dim=-1)
